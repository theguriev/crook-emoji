...
//! it wanted to: it has no network, no filesystem, no clock and no thread, and
//! asks for none of them. They are wrapped here rather than called from the
//! logic for one reason — everything else in this crate then builds and runs
//! on an ordinary machine, so which emoji a tab gets is decided by something
...
/// earlier. `tab.row.mark` holds one thing, so the order is what decides
